            ...
            steps {
                script {
                    docker.image('snap-build-server.tilaa.cloud/xvfb:1.0').withRun() { c ->
                        docker.image("snap-build-server.tilaa.cloud/${params.dockerTagName}").inside("--group-add 1006 --link ${c.id} -e DISPLAY=${c.id}:0 -v /data/ssd/testData/test-user/qftest:/data/Products/qftest -v /opt/qftest/licence:/home/snap/qftest/licence") {
