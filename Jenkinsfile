    ...
    stages {
        stage('GUI Tests') {
            ...
            steps {
                ...
                sh "export qftest_data_dir=/data/ssd/testData/snap-gui-tests/qftest && export qftest_snap_install_dir=/home/snap/snap/ && export qftest_snap_user_dir=/home/snap/.snap && xvfb-run /usr/local/bin/qftest -batch -runlog $WORKSPACE/qftest_logs -report $WORKSPACE/qftest_report -suitesfile $WORKSPACE/${params.testFileList}"
            }
            post {
                always {
                    sh "ls -l $WORKSPACE"
